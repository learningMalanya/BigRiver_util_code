...
X = Matshape(35, -60, 70, 0);
L = Matshape(65, -35, 0, -65);
gap = X.x-Y.w;
...
plot!(
    rec_L,
    fillcolor = plot_color(my_red, 0.4), 
    linecolor = my_white,
    linewidth = my_linewidth
)
...
annotate!([
        ...
        # (B.w/2+B.x, B.h/2+B.y, ("B", mat_font_size, :black, :center, mat_font)),
])

# Descriptions
annotate!([
        (Y.w/2, Y.h/2-9, ("high-throughput output", dim_font_size, :grey, :center, dim_font)), 
        (X.w/2+X.x, X.h/2-9, ("sample\ncharacteristics", dim_font_size, :grey, :center, dim_font)),
        (Z.w/2, Z.h/2+Z.y-9, ("response characteritics", dim_font_size, :grey, :center, dim_font)),
        # (B.w/2+B.x, B.h/2+B.y-9, ("coefficients", dim_font_size, :grey, :center, dim_font)),
])

# Dimensions
annotate!([
        (Y.w+X.w+gap+4, Y.h/2, ("n", dim_font_size, :black, :center, dim_font)), 
        (Y.w+X.w+gap+4, B.h/2+B.y, ("q", dim_font_size, :black, :center, dim_font)),
        (Z.w/2, Y.h+Z.h-gap-4, ("m", dim_font_size, :black, :center, dim_font)),
        # (B.w/2+B.x, Y.h+Z.h-gap-4, ("p", dim_font_size, :black, :center, dim_font)),
])
...
Z_box = Matshape(65,-35,0,-65)
Z_col = Matshape(5,-35,55,-65);
Z_row = Matshape(65,-5,0,-75);

B_box = Matshape(30,-35,70,-65);
B_col = Matshape(5,-35,90,-65);
B_row = Matshape(30,-5,70,-75);
...
rec_Z_box, rec_Z_col, rec_Z_row = matcolrow(Z_box, Z_col, Z_row);
rec_B_box, rec_B_col, rec_B_row = matcolrow(B_box, B_col, B_row);
...
# Plot Z matrix #
#################
plot_matcolrow!(rec_Z_box, rec_Z_col, rec_Z_row, my_blue, my_linewidth)


#################
# Plot B matrix #
#################
plot_matcolrow!(rec_B_box, rec_B_col, rec_B_row, my_red, my_linewidth, my_red)
...
annotate!([
        ...
        nntt_mat(B_box, B_col, B_row, "B", mat_font_size, :black, :center, mat_font),
])
...
annotate!([
        ...
        (B_box.w/2+B_box.x, Y_box.h+Z_box.h-gap-4, ("p", dim_font_size, :black, :center, dim_font)),
])
